...
![八种交互动效设计示意：磁吸、液态形变、3D 视差、滚动聚焦、流体 Tab、图片展开、手势驱动与物理碰撞](docs/images/motion-overview.png)

从卡片吸附到页面转场，展示本技能可以指导设计与实现的八类交互。图中的轨迹、分层和中间状态用来解释运动关系。

### 图片展开：保持同一内容的空间连续性

![图片展开三阶段设计示意：缩略图、展开中、全屏查看](docs/images/image-expand.png)

缩略图 → 展开中 → 全屏查看。重点是让用户理解“同一张图片正在展开”，而不是突然换到另一个页面。实际实现还需处理裁切、手势取消和返回原位，详见 [图片展开实现参考](references/motion-patterns.md#6-图片展开--image-expand)。

以上为 AI 生成的原创应用设计示意图，用于解释技能覆盖的交互场景；并非原作者视频截图，也不是已运行应用的截图或动画录屏。静态图不能验证跟手、帧率或物理反馈，实际效果以项目实现和测试为准。
